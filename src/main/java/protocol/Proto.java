package com.alex.jungletiger.protocol;
...
public class Proto {
    ...
    public static byte[] build_fixed_int(int size, long value) {
        ...
        if (size == 8) {
            ...
            LoggerFactory.getLogger(Proto.class).error("Encoding int["+size+"] "+value+" failed!");
            return null;
        }
        return packet;
    }
    ...
    public long get_lenenc_int() {
        ...
        if (size == 0) {
            LoggerFactory.getLogger(Proto.class).error("Decoding int at offset "+offset+" failed!");
            return -1;
        }
